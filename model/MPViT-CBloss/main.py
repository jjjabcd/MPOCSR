...
class CBLoss(nn.Module):
    def __init__(self, samples_per_cls, num_of_classes, beta=0.9, gamma=2):
    ...
    def forward(self, inputs, targets):
        ...
        weights = weights / np.sum(weights) * self.num_of_classes

        weights = torch.tensor(weights).float().cuda()
        weights = weights.unsqueeze(0)
        weights = weights.repeat(targets_onehot.shape[0], 1) * targets_onehot
        ...
        weighted_loss = torch.where(positive_label_mask, weights * loss,
                                 (1.0 - weights) * loss)
...
def parse_option():
    ...
    parser.add_argument('--cfg', default=None,type=str, metavar="FILE", help='path to config file', )
    parser.add_argument(
        "--opts",
...
def main(config) :
    ...
    samples_per_cls = torch.load('../../Data/200w_word_freq_train.pth')
    criterion = CBLoss(samples_per_cls, len(word_map))
    max_accuracy = 0.0
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        train_one_epoch(config, encoder, encoder_optimizer, decoder, decoder_optimizer, criterion, data_loader_train, epoch, mixup_fn, encoder_lr_scheduler)
...
def train_one_epoch(config, encoder, encoder_optimizer, decoder, decoder_optimizer, criterion, train_loader, epoch, mixup_fn, encoder_lr_scheduler):
    encoder.train()
    decoder.train()
    ...
    for idx, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        for index, caplen in enumerate(caplens_np):
            ...
            caps_target += caps_np[index][1:caplen]  # <start> and the extra pad after it were removed
